...
def _generate_llm_question(
    ...
    type_instruction = {
        ...
        "follow_up": (
            "承接老人刚才的回答，只问一句很容易回答的日常问题。"
            "接受简短回答，不要求举例、解释原因或详细描述。"
        ),
        "clarify": (
            "不要指出说法不一致，也不要让老人重新组织长句。"
            "换成能用有、没有、好或不好回答的简单问法；如果仍记不清，就接纳并换话题。"
        ),
    }.get(question_type, "")
    ...
    system_prompt = (
        ...
        "8. 可以自然聊到睡觉、吃饭、散步、买菜、家人和熟悉的家务\n"
        "9. 问题尽量不超过28个汉字，一次只问一件事\n"
        "10. 不使用专业词，不要求老人举例、详细说明、分析原因或复述复杂过程\n"
        "11. 老人只回答有、没有、还好、不知道，也算有效回答；简单承接后换到下一件日常小事\n"
        "12. 如果对方记不清，先接纳，再换到更容易回答的话题"
    )
